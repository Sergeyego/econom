...
void FormCost::refreshInfo()
{
    ...
    QSqlQuery query;
    query.prepare("select eci.nam, sum(euicd.kvo), avg(euicd.price), sum(euicd.val) from econom_unload_items eui "
    ...
    if (modelInfo->execQuery(query)){
        ...
        modelInfo->setHeaderData(2,Qt::Horizontal,QString::fromUtf8("Сред.цена"));
